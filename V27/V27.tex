...
\begin{figure}
  ...
  \caption{Fit und Messwerte zur Kalibrierung des Elektromagneten.}
  \label{abb:1}
\end{figure}
...
Das Dispersionsgebiet wird mit der Formel \eqref{eq:dis} berechnet, vergleiche \ref{tab3}. Die Wellenlängenänderung $\symup{\delta}\lambda$ kann somit durch die Formel
...
mit Hilfe der Ausgleichsrechnung zur Kalibrierung des Magnetfeld berechnet
...
\begin{subfigure}[c]{0.5\textwidth}
  ...
  \subcaption{B$\neq$0}
\end{subfigure}
\caption{Aufspaltung der roten Linie beim $\sigma$-Übergang.}
\end{figure}
...
Dispersiongebiet mit Hilfe der Gleichung \eqref{eq:dis} berechnet werden, vergleiche Tabelle \ref{tab3}.
...
Nun können durch die Gleichung \eqref{eq:4} die Landé-Faktoren berechnet werden.
...
Theoriewert zwar nicht in der Messungenauigkeit, aber in dem $2\sigma$-Intervall.
...
Eine weitere große Fehlerquelle ist das Ablesen des Stroms am Elektromagneten, da
dieses nur sehr ungenau machbar war, hierbei ensteht ein Ablesefehler von ungefähr
\SI{0.25}{\ampere}. Durch den Fehler liegen die Messwerte höchstwahrscheinlich
in der $\sigma$-Umgebung, da der Fehler dadurch sehr viel größer wird.
